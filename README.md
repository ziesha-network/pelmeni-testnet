...
Now if you want to join the `groth-2` testnet, you first have to initialize your
...
Example to initialize a node with 2 bootstrap nodes `65.108.193.133:8765` on the `groth-2` network:

```
bazuka init --network groth-2 --bootstrap 65.108.193.133:8765 --mnemonic "YOUR OLD MNEMONIC PHRASE"
```

If you don't have a mnemonic phrase and you would like `bazuka` to generate one for you:

```
bazuka init --network groth-2 --bootstrap 65.108.193.133:8765
```
...
4. Run `zoro` beside your node: (This will use your Nvidia GPU for mining!)
   ...
   ```sh
   zoro --node 127.0.0.1:8765 --seed "SEED PHRASE FOR THE EXECUTOR ACCOUNT" --network groth-2 \
